...
export default {
  ...
  created() {
    ...
    this.getProComments(this.goodsId, this.limit)
  },
  ...
  methods: {
    ...
    async getDetail(goodsId) {
      const { data: { detail } } = await getProDetail(goodsId)
      this.detail = detail
      this.images = detail.goods_images
    },
    async getProComments(goodsId, limit) {
      const { data: { list, total } } = await getProComments(goodsId, limit)
    ...
    loginCofirm(){
        ...
        // 弹窗。.then的回调函数。是方法确认时候的时间，.catch是拒绝的方法
        this.$dialog.confirm({
          title: '温馨提示',
          ...
          // 跳转回来成功，需要带上参数
          // this.$route.fullpath(包含查询参数),replace不是跳转,直接替换
          this.$router.replace({
            path: '/login',
            query: {
